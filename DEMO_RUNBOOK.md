...
5. If this is a fresh environment, create Azure resources in the Portal first
  (same resource group):
  - Azure AI Services account (kind `AIServices`) with a model deployment
    (for example `gpt-4o`)
  - Azure AI Foundry project under that AI Services account
  - Azure AI Search service
  - Storage account + blob container (for blob URL ingestion)

6. Bootstrap `.env` from those existing Azure resources:

  ```bash
  RG=<your-resource-group> bash infra/bootstrap.sh
  ```

  `infra/bootstrap.sh` also ensures blob readiness:
  - Creates the blob container (default: `data`) if it does not exist.
  - Uploads files from local `sampledata/` into that container (no overwrite).

  Optional (also write GitHub Codespaces secrets):

  ```bash
  RG=<your-resource-group> PUSH_CODESPACES=1 GH_REPO=<owner/repo> bash infra/bootstrap.sh
  ```
...
### 9.1 First-time deployment checklist

1. Confirm `.env` exists (generated by bootstrap in step 1).
2. Export runtime variables from `.env` into your shell:

```bash
set -a; source .env; set +a
```

3. Deploy infrastructure + API image:

```bash
RG=med-doc ./infra/deploy.sh
# → https://<app>.<region>.azurecontainerapps.io/docs
```

`infra/deploy.sh` now auto-creates names when omitted:
- `ACA_ENV` defaults to `<rg>-env`
- `APP_NAME` defaults to `<rg>-api`
- `ACR_NAME` is auto-generated and uniqueness-checked
- `LOCATION` defaults to RG location (or `eastus` for a new RG)

Optional overrides (only if you want custom names):

```bash
RG=med-doc LOCATION=eastus APP_NAME=my-demo-api ACA_ENV=my-demo-env ACR_NAME=mydemoacr123 ./infra/deploy.sh
```

4. Managed identity access is handled by `infra/deploy.sh`:
   - Always assigns `AcrPull` on the created/selected ACR.
   - If `BLOB_CONTAINER_URL` is set, assigns `Storage Blob Data Reader` on that
     storage account.
   - If `AZURE_AI_PROJECT_ENDPOINT` is set, attempts to assign `Azure AI Developer`
     on that Foundry project scope (set `FOUNDRY_RG` only when project RG differs).

5. Verify health + docs:

```bash
APP_NAME=med-doc-api
FQDN=$(az containerapp show -g med-doc -n "$APP_NAME" --query properties.configuration.ingress.fqdn -o tsv)
curl -i "https://$FQDN/healthz"
curl -i "https://$FQDN/docs"
```

### 9.2 Create a new revision only (no rebuild)

If you only want a fresh revision rollout (for example after env updates),
force a no-op env change:

```bash
APP_NAME=med-doc-api
az containerapp update -g med-doc -n "$APP_NAME" --set-env-vars RESTART_AT="$(date +%s)"
```
