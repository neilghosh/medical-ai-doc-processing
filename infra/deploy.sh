...
ENV_PAIRS=()
for v in ENDPOINT_URL DEPLOYMENT_NAME AZURE_OPENAI_API_KEY AZURE_SEARCH_ENDPOINT AZURE_SEARCH_KEY AZURE_SEARCH_QUERY_KEY AZURE_SEARCH_INDEX_NAME AZURE_AI_PROJECT_ENDPOINT AGENT_MODEL_DEPLOYMENT ORCHESTRATOR_AGENT_ID API_KEY; do
  [ -n "${!v:-}" ] && ENV_PAIRS+=("$v=${!v}")
done
